...
export default function RecommendationsPage() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useUserRecommendations(undefined, 20);

  // Debug: Log the recommendations data
  React.useEffect(() => {
    if (allRecommendations) {
      console.log("📊 [RECOMMENDATIONS] Received data:", allRecommendations);
      console.log(
  ...
  const filteredRecommendations = (allRecommendations || []).filter((rec) => {
    ...
    // Map backend types to frontend categories correctly
    if (selectedCategory === "weak") {
      // Show weak areas that need urgent attention (high priority)
      return rec.type === "weak" || rec.priority === "high";
    }
    if (selectedCategory === "practice") {
      // Show areas that need practice (medium priority)
      return rec.type === "practice" || rec.priority === "medium";
    }
    if (selectedCategory === "advanced") {
      // Show strong areas for advanced challenges (low priority)
      return rec.type === "advanced" || rec.priority === "low";
    }
    return true;
  });
    ...
    <GameLayout>
      <div className="space-y-8">
        ...
        >
          ...
          />
          <h1 className="text-responsive-3xl font-bold text-gray-900 dark:text-white mb-2">
            AI Recommendations 🎯
          </h1>
        ...
        >
          <div className="flex flex-wrap gap-2">
            {(["all", "weak", "practice", "advanced"] as const).map(
              (category) => (
                <button
                  key={category}
            ...
            <div className="text-responsive-2xl font-bold text-gray-900 dark:text-white">
              {
                (allRecommendations || []).filter(
                  (r: any) => r.type === "weak" || r.priority === "high"
                ).length
              }
            </div>
            <div className="text-responsive-sm text-gray-600 dark:text-gray-400">
            ...
            <div className="text-responsive-2xl font-bold text-gray-900 dark:text-white">
              {(() => {
                ...
                const avg = Math.round(
                  arr.reduce((sum, r) => sum + (r.confidence || 0), 0) /
                    arr.length
                );
                return `${avg}%`;
              })()}
          ...
          <div className="game-card-theme p-4 text-center">
            ...
            <div className="text-responsive-2xl font-bold text-gray-900 dark:text-white">
              {(allRecommendations || []).reduce(
                (sum, r: any) => sum + (r.metadata?.estimatedTime ? 5 : 0),
                0
              )}
            </div>
            <div className="text-responsive-sm text-gray-600 dark:text-gray-400">
        ...
        >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {recommendation.priority && (
                        <span
                          className={`px-2 py-1 rounded-full text-responsive-xs font-medium ${
                    ...
                    <p className="text-responsive-base text-gray-300 mb-2">
                      {recommendation.description}
                    </p>
                    {recommendation.reason && (
                      <p className="text-responsive-sm text-blue-300 mb-2 italic">
                        💡 {recommendation.reason}
                      </p>
                    )}
              ...
              {recommendation.metadata &&
                Object.keys(recommendation.metadata).length > 0 && (
                  <div className="mb-4 p-3 bg-gray-800/50 rounded-lg">
                    <h4 className="text-responsive-sm font-semibold text-white mb-2">
                      Details:
                    </h4>
                      ...
                      {recommendation.metadata.topicId &&
                        !recommendation.metadata.quizId && (
                          <button
                            onClick={() =>
                              router.push(
                                `/subjects/${recommendation.metadata.subjectId}/topics/${recommendation.metadata.topicId}`
                              )
                            }
                      ...
                      {recommendation.metadata.subjectId &&
                        ...
                        !recommendation.metadata.quizId && (
                          <button
                            onClick={() =>
                              router.push(
                                `/subjects/${recommendation.metadata.subjectId}`
                              )
                            }
          ...
          {filteredRecommendations.length === 0 && !isLoading && (
            <div className="text-center py-8 text-gray-400">
              ...
              <p>
                No recommendations yet. Complete a quiz to get personalized
                suggestions.
              </p>
            </div>
          )}
