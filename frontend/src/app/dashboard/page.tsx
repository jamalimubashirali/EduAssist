...
import {
  QuickQuizButton,
  ChallengeQuizButton,
} from "@/app/components/quiz/StartQuizButton";
...
export default function Dashboard() {
  ...
  const {
    ...
    authUser?.id && !isLoading ? authUser.id : undefined
  );

  // Load streak data and update gamification store
  const setGamificationState = useGamificationStore(
    (state) => state.setGamificationState
  );
  ...
  const { data: popularSubjects, isLoading: subjectsLoading } =
    usePopularSubjects(5);

  const { data: recommendedQuizzes, isLoading: quizzesLoading } =
    useRecommendedQuizzes(authUser?.id || "");

  const { data: quizRecommendations, isLoading: quizRecLoading } =
    useQuizRecommendations(5, authUser?.id);
  ...
  const handleRecommendationClick = async (rec: any) => {
    try {
      ...
      console.warn(
        "Failed to start recommendation quiz, redirecting to quiz arena",
        e
      );
      router.push("/quiz");
    }
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-responsive-base text-gray-400">
                {dashboardContent?.focusAreas &&
                dashboardContent.focusAreas.length > 0
                  ? `Focus on ${dashboardContent.focusAreas
                      .slice(0, 2)
                      .join(" and ")} today!`
                  : "Ready to level up your learning today?"}
              </p>
              {dashboardContent?.weakSubjects &&
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {dashboardContent.weakSubjects
                      ...
                      .map((subject: any, index: number) => (
                        <span
                          key={index}
                          className="px-2 py-1 bg-orange-500/20 text-orange-300 rounded-full text-responsive-xs"
                        >
                          Improve {subject.subject_name} (
                          {Math.round(subject.score_percentage)}%)
                        </span>
                      ))}
                  </div>
                )}
              {dashboardContent?.strongSubjects &&
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {dashboardContent.strongSubjects
                      ...
                      .map((subject: any, index: number) => (
                        ...
                        >
                          Strong in {subject.subject_name} (
                          {Math.round(subject.score_percentage)}%)
                        </span>
                      ))}
                  </div>
        ...
        >
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <button
                onClick={() =>
                  handleQuickAction("Study Session", 75, "/subjects")
                }
                className="game-card p-4 hover:scale-105 transition-transform duration-200 text-left"
              >
                <div className="flex items-center gap-3 mb-3">
              ...
              <button
                onClick={() =>
                  handleQuickAction("View Progress", 0, "/progress")
                }
                className="game-card p-4 hover:scale-105 transition-transform duration-200 text-left"
              >
                <div className="flex items-center gap-3 mb-3">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-responsive-2xl font-primary font-semibold text-white">
                Active Quests
              </h2>
            </div>
            <button
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-secondary font-semibold text-white">
                  Popular Subjects
                </h2>
              </div>
              <button
              ...
              {quizzesLoading
                ...
                : recommendedQuizzes.slice(0, 3).map((quiz) => (
                    <button
                      key={quiz.id}
                      onClick={() =>
                        router.push(`/quiz/instructions/${quiz.id}`)
                      }
                      className="game-card p-4 hover:scale-105 transition-transform duration-200 text-left"
                    >
                      <div className="flex items-center gap-3 mb-3">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-primary font-semibold text-white">
                Recent Achievements
              </h2>
            </div>
            <button
            ...
            {isBadgesLoading ? (
              ...
              Array(3)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="flex-shrink-0 w-20 h-20 bg-gray-700 rounded-full animate-pulse"
                  ></div>
                ))
              ...
              <>
                ...
                {badges
                  .filter((b) => !unlockedBadges.find((ub) => ub.id === b.id))
                  .slice(0, 3)
                  .map((badge) => (
                    <div key={badge.id} className="flex-shrink-0">
                      <BadgeCard
