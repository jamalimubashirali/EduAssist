...
export class LoggerMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction): void {
    ...
    // Store reference to logger for use in closures
    ...
    res.end = function (chunk?: any) {
      ...
      // Log response
      const logMessage = `${statusEmoji} RESPONSE: ${method} ${originalUrl} - Status: ${statusCode} ${statusMessage} - Duration: ${duration}ms`;
      ...
      if (statusCode >= 400 || process.env.LOG_LEVEL === 'debug') {
        try {
          const sanitizedResponse = logger.constructor.prototype.sanitizeResponseBody?.call(logger.constructor.prototype, responseBody) || responseBody;
