...
export class UsersService {
  ...
  async updateOnboarding(id: string, payload: any): Promise<User> {
    ...
    if (payload.step) {
      ...
      updates['onboarding.status'] =
        payload.step === 'COMPLETION-SUMMARY' ? 'COMPLETED' : 'IN_PROGRESS';
  ...
  async submitAssessment(body: {
    ...
    try {
      ...
      const questionsWithData =
        await this.questionsService.getQuestionsWithSubjectTopicData(
          questionIds,
        );
      console.log(
        '[submitAssessment] Retrieved questions with data:',
        questionsWithData.length,
      );

      // Create a map for quick lookup
      const questionMap = new Map(
        questionsWithData.map((q) => [q._id.toString(), q]),
      );
      ...
      for (const answer of body.answers) {
        if (!Types.ObjectId.isValid(answer.question_id)) {
          console.log(
            '[submitAssessment] Invalid question_id, skipping',
            answer.question_id,
          );
          continue;
        }
        ...
        if (!question) {
          console.log(
            '[submitAssessment] question not found, skipping',
            answer.question_id,
          );
          continue;
        }
        ...
        console.log('[submitAssessment] question checked', {
        ...
        const subjectId = question.subjectId._id
          ? question.subjectId._id.toString()
          : question.subjectId.toString();
        const topicId = question.topicId._id
          ? question.topicId._id.toString()
        ...
        if (
          ...
          console.log(
            '[submitAssessment] Invalid ObjectId format, skipping performance update',
            {
              subjectId,
              topicId,
              questionId: answer.question_id,
        ...
        this.questionsService
          .updateQuestionStats(answer.question_id, isCorrect, answer.time_taken)
          .catch((err) =>
            console.log(
              '[submitAssessment] Error updating question stats:',
              err.message,
            ),
          );
      ...
      const overallScore =
        totalQuestions > 0
          ...
          : 0;
      console.log('[submitAssessment] score computed', {
        totalQuestions,
        correctAnswers,
        overallScore,
      });
      ...
      const subjectAnalysis = await Promise.all(
        Array.from(subjectStats.entries()).map(async ([subjectId, stats]) => {
          ...
          const subjectQuestions = questionDetails.filter(
          ...
          for (const question of subjectQuestions) {
            ...
            if (!topicPerformance.has(topicId)) {
              ...
              const topicName =
                questionData?.topicId?.topicName || 'Unknown Topic';
          ...
          return {
            ...
            topic_breakdown: Array.from(topicPerformance.entries()).map(
              ([topicId, performance]) => ({
                topic_id: topicId,
      ...
      console.log('[submitAssessment] subjectAnalysis ready', {
      ...
      const weakSubjects = subjectAnalysis.filter(
        (s) => s.score_percentage < 60,
      );
      const strongSubjects = subjectAnalysis.filter(
        (s) => s.score_percentage >= 80,
      );
      const recommendations = {
        ...
        focus_areas: [
          ...
          ...(weakSubjects.length === 0
            ? ['Continue strengthening all subjects']
            : []),
        ],
        priority_subjects:
          weakSubjects.length > 0
      ...
      console.log('[submitAssessment] results assembled', {
      ...
      const attemptId: string | null = null;
      console.log(
        '[submitAssessment] Skipping attempt record creation for assessment (schema requires quizId/topicId)',
      );

      // Store assessment results for performance tracking
      try {
        console.log('[submitAssessment] updating performance records', {
        ...
        for (const detail of questionDetails) {
          ...
          if (
            !Types.ObjectId.isValid(detail.topicId) ||
            !Types.ObjectId.isValid(detail.subjectId)
          ) {
            console.log(
              '[submitAssessment] Skipping performance update for invalid ObjectIds',
              {
          ...
          try {
            ...
            console.log(
              '[submitAssessment] Error updating individual performance record',
              {
        ...
        console.log(
          '[submitAssessment] Error storing performance data (continuing)',
          error?.message || error,
        );
      ...
      console.log('[submitAssessment] generating intelligent recommendations', {
        attemptId,
      });
      ...
      console.log('[submitAssessment] intelligent recommendations generated', {
        count: intelligentRecommendations.length,
      });
      ...
      console.log(
        '[submitAssessment] gamification updated',
        gamificationUpdate,
      );
      ...
      console.log('[submitAssessment] failed', {
  ...
  async getUserPreferences(
    userId: string,
  ): Promise<{ preferences: { id: string; name: string }[] }> {
    if (!Types.ObjectId.isValid(userId)) {
      throw new NotFoundException('User not found');
    }
    ...
    try {
      ...
      for (const subject of assessmentResults.subject_analysis) {
        if (subject.score_percentage < 60) {
          ...
          if (!Types.ObjectId.isValid(subject.subject_id)) {
            console.log(
              '[generateIntelligentRecommendations] Invalid subject_id, skipping',
              subject.subject_id,
            );
            continue;
          }
          ...
          try {
            ...
            console.error(
              `Error generating recommendation for weak subject ${subject.subject_id}:`,
              error,
            );
          }
        }
      ...
      for (const subject of assessmentResults.subject_analysis) {
        if (subject.score_percentage >= 80) {
          ...
          if (!Types.ObjectId.isValid(subject.subject_id)) {
            console.log(
              '[generateIntelligentRecommendations] Invalid subject_id for advancement, skipping',
              subject.subject_id,
            );
            continue;
          }
          ...
          try {
            ...
            console.error(
              `Error generating advancement recommendation for subject ${subject.subject_id}:`,
              error,
            );
          }
        }
      ...
      if (assessmentResults.recommendations?.study_plan) {
        recommendations.push({
          ...
          description:
            'AI-generated study plan based on your assessment performance',
          ...
          metadata: {
            dailyQuestions:
              assessmentResults.recommendations.recommended_daily_questions,
            studyTimePerDay:
              assessmentResults.recommendations.study_time_per_day,
            focusAreas: assessmentResults.recommendations.focus_areas,
            prioritySubjects:
              assessmentResults.recommendations.priority_subjects,
  ...
  private async updateGamificationFromAssessment(
    userId: string,
    assessmentResults: any,
  ): Promise<any> {
    try {
      ...
      if (totalQuizzesAttempted > 1) {
        averageScore =
          ((user.averageScore || 0) * (totalQuizzesAttempted - 1) +
            assessmentResults.overall_score) /
  ...
  async saveOnboardingProgress(
    ...
    try {
      ...
      if (progressData.current_step)
        updates['onboarding.current_step'] = progressData.current_step;
      if (progressData.total_steps)
        updates['onboarding.total_steps'] = progressData.total_steps;
      if (progressData.data) updates['onboarding.data'] = progressData.data;
      if (progressData.current_question_index)
        updates['onboarding.current_question_index'] =
          progressData.current_question_index;
  ...
  private calculateLearningAnalytics(
    questionDetails: any[],
    answers: any[],
    overallScore: number,
    subjectAnalysis: any[],
    ...
    const totalTime = answers.reduce(
      (sum, answer) => sum + (answer.time_taken || 0),
      0,
    );
    ...
    const thoughtfulAnswers = thinkingTimes.filter(
      (t) => t >= 10 && t <= 60,
    ).length;
    ...
    return {
      studyPatterns: {
        ...
        thoughtfulResponseRate: Math.round(
          (thoughtfulAnswers / answers.length) * 100,
        ),
      ...
      performanceMetrics: {
        ...
        strongSubjects: subjectAnalysis
          .filter((s) => s.score_percentage >= 75)
          .map((s) => s.subject_name),
        improvementAreas: subjectAnalysis
          .filter((s) => s.score_percentage < 60)
          .map((s) => s.subject_name),
      },
      adaptiveRecommendations: {
        recommendedDifficulty: recommendedDifficulty,
        suggestedStudyTime: this.calculateOptimalStudyTime(
          overallScore,
          learningStyle,
        ),
  ...
  private calculateAccuracyTrend(questionDetails: any[]): string {
    const firstHalf = questionDetails.slice(
      0,
      Math.floor(questionDetails.length / 2),
    );
    const secondHalf = questionDetails.slice(
      Math.floor(questionDetails.length / 2),
    );

    const firstHalfAccuracy =
      (firstHalf.filter((q) => q.isCorrect).length / firstHalf.length) * 100;
    const secondHalfAccuracy =
      (secondHalf.filter((q) => q.isCorrect).length / secondHalf.length) * 100;
  ...
  private getPersonalizedApproach(
    learningStyle: string,
    score: number,
  ): string[] {
  ...
  private async updateOnboardingAssessmentData(
    userId: string,
    assessmentResults: any,
  ): Promise<void> {
    try {
      const updates = {
        'onboarding.assessmentData': {
          ...
          subjectBreakdown: assessmentResults.subject_analysis.reduce(
            (acc: any, subject: any) => {
          ...
          strongAreas: assessmentResults.subject_analysis
            ...
            .map((s: any) => s.subject_name),
          learningStyle:
            assessmentResults.learning_analytics?.studyPatterns
              ?.learningStyle || 'BALANCED',
          recommendedPath:
            this.generateRecommendedLearningPath(assessmentResults),
          initialDifficulty:
            assessmentResults.learning_analytics?.adaptiveRecommendations
        ...
        'onboarding.learningPreferences': {
          ...
          weeklyGoal:
            assessmentResults.recommendations?.recommended_daily_questions || 5,
        },
        'onboarding.progressMetrics': {
          ...
          consistencyScore:
            assessmentResults.learning_analytics?.performanceMetrics
              ?.consistencyScore || 50,
      ...
      console.log(
        `Enhanced onboarding assessment data updated for user ${userId}`,
      );
    } catch (error) {
  ...
  private generateRecommendedLearningPath(assessmentResults: any): string {
    ...
    const weakSubjects = assessmentResults.subject_analysis.filter(
      (s: any) => s.score_percentage < 60,
    );
  ...
  private calculateEngagementLevel(assessmentResults: any): string {
    ...
    // Medium engagement: decent performance or time
      ...
      score >= 50 ||
      (avgTimePerQuestion >= 10 && avgTimePerQuestion <= 180)
    ) {
      return 'MEDIUM';
    }
  ...
  async getUserStats(userId: string) {
    ...
    const percentile =
      rank > 0
        ? Math.max(1, 100 - Math.round((rank / allUsers.length) * 100))
        : 0;
  ...
  async getUserQuests(userId: string) {
    ...
    if (Array.isArray(performances) && performances.length > 0) {
      const averageScore =
        performances.reduce(
          (sum: any, p: any) => sum + (p.averageScore || 0),
          0,
        ) / performances.length;
  ...
  async getUserBadges(userId: string) {
    ...
    if (user.totalQuizzesAttempted > 0) {
      badges.push({
        ...
        rarity: 'common',
        category: 'milestone',
        unlockedAt: user.unlockedBadges?.includes('first-quiz')
          ? user.createdAt
          : null,
        progress: 1,
        maxProgress: 1,
    ...
    if (totalQuizzes >= 10) {
      badges.push({
        ...
        rarity: 'common',
        category: 'completion',
        unlockedAt: user.unlockedBadges?.includes('quiz-explorer')
          ? user.updatedAt
          : null,
    ...
    if (currentStreak >= 7) {
      badges.push({
        ...
        rarity: 'rare',
        category: 'consistency',
        unlockedAt: user.unlockedBadges?.includes('week-warrior')
          ? user.updatedAt
          : null,
    ...
    if (user.averageScore >= 90) {
      badges.push({
        ...
        rarity: 'epic',
        category: 'performance',
        unlockedAt: user.unlockedBadges?.includes('perfectionist')
          ? user.updatedAt
          : null,
  ...
  async getUserLeaderboardPosition(userId: string) {
    ...
    return {
      ...
      percentile: Math.max(
        1,
        100 - Math.round(((userIndex + 1) / allUsers.length) * 100),
      ),
  ...
  private generateComprehensiveRecommendations(
    ...
    const recommendations = {
      ...
      subject_recommendations:
        this.generateSubjectRecommendations(subjectAnalysis),

      // Learning path recommendations
      learning_path: this.generateLearningPath(
        proficiency,
        overallScore,
        subjectAnalysis,
      ),
      ...
      practice_recommendations: this.generatePracticeRecommendations(
        questionDetails,
        subjectAnalysis,
      ),
      ...
      time_management:
        this.generateTimeManagementRecommendations(questionDetails),

      // Study strategy recommendations
      study_strategy: this.generateStudyStrategyRecommendations(
        overallScore,
        proficiency,
      ),
  ...
  private getPerformanceInsight(score: number): string {
    if (score >= 90)
      return 'Exceptional performance! You demonstrate mastery of the concepts.';
    if (score >= 80)
  ...
  private generateLearningPath(
    ...
    const averageSubjects = subjectAnalysis.filter(
      (s) => s.score_percentage >= 60 && s.score_percentage < 80,
    );
    ...
    return {
      ...
      estimated_time_to_next_level: this.estimateTimeToNextLevel(
        overallScore,
        proficiency,
      ),
  ...
  private generatePracticeRecommendations(
    ...
    return {
      ...
      daily_practice_goal: Math.max(
        5,
        Math.ceil(incorrectQuestions.length / 2),
      ),
      weekly_practice_goal: Math.max(20, incorrectQuestions.length * 2),
      practice_strategy: this.getPracticeStrategy(
  ...
  private generateTimeManagementRecommendations(questionDetails: any[]): any {
    const avgTimePerQuestion =
      questionDetails.reduce((sum, q) => sum + q.timeTaken, 0) /
      questionDetails.length;
    const slowQuestions = questionDetails.filter(
      (q) => q.timeTaken > avgTimePerQuestion * 1.5,
    );
    ...
    return {
      ...
      recommended_practice_time: Math.max(
        15,
        Math.ceil(avgTimePerQuestion * 10),
      ),
  ...
  private generateStudyStrategyRecommendations(
    overallScore: number,
    proficiency: string,
  ): any {
    const strategies = {
    ...
    return {
      recommended_strategies:
        strategies[proficiency as keyof typeof strategies] ||
  ...
  private generateImprovementAreas(
    ...
    return {
      ...
      specific_topics: Array.from(incorrectTopics.entries()).map(
        ([key, count]) => {
          const [subjectId, topicId] = key.split('-');
          const subject = subjectAnalysis.find(
          ...
          if (subject && subject.topic_breakdown) {
            const topicInfo = subject.topic_breakdown.find(
              (t: any) => t.topic_id === topicId,
            );
      ...
      improvement_timeline: this.generateImprovementTimeline(
        weakSubjects.length,
      ),
    };
  }
  ...
  private generateSuccessFactors(
    ...
    const consistentSubjects = subjectAnalysis.filter(
      (s) => s.score_percentage >= 60,
    );
    ...
    return {
      ...
      consistency_indicators: {
        ...
        consistency_rate: Math.round(
          (consistentSubjects.length / subjectAnalysis.length) * 100,
        ),
      },
      growth_indicators: {
  ...
  private generateStudySchedule(score: number, proficiency: string): any {
    ...
    return {
      ...
      session_duration: Math.round(
        baseTime / Math.max(3, Math.ceil(7 * adjustment)),
      ),
  ...
  private getLearningTechniques(proficiency: string): string[] {
    ...
    return (
      techniques[proficiency as keyof typeof techniques] ||
  ...
  private getSuccessFactors(score: number): string[] {
    ...
    if (score >= 70)
      return ['Basic understanding', 'Some practice', 'Room for improvement'];
