...
export class PerformanceService {
  ...
  async updatePerformance(
    ...
    try {
      ...
      const totalTimeSpent = recentAttempts.reduce(
      ...
      const difficultyPerformance =
        this.calculateDifficultyPerformance(recentAttempts);
      ...
      performance.masteryLevel = this.calculateMasteryLevel(
        averageScore,
  ...
  async getUserGamificationStats(userId: string): Promise<any> {
    try {
      ...
      const averageMastery =
        performances.reduce((sum, p) => sum + p.masteryLevel, 0) / totalTopics;

      // Identify strong and weak areas
      const sortedByMastery = [...performances].sort(
      ...
      const weakAreas = sortedByMastery
        .slice(-3)
        .reverse()
        .map((p) => ({
          topic: (p.topicId as any)?.topicName || 'Unknown Topic',
      ...
      const recentAttempts = await this.attemptModel
        ...
        .exec();
      const recentProgress = recentAttempts.map((attempt) => ({
      ...
      const subjectBreakdown = performances.reduce((acc, p) => {
        const subjectName =
          (p.subjectId as any)?.subjectName || 'Unknown Subject';
        if (!acc[subjectName]) {
          acc[subjectName] = {
            topicCount: 0,
            averageMastery: 0,
      ...
      Object.keys(subjectBreakdown).forEach((subject) => {
        ...
        subjectBreakdown[subject].averageMastery = Math.round(
          subjectBreakdown[subject].averageMastery / count,
        );
        subjectBreakdown[subject].averageScore = Math.round(
          subjectBreakdown[subject].averageScore / count,
        );
      });
    ...
    try {
      ...
      const dailyStats = recentAttempts.reduce((acc, attempt) => {
        ...
        if (attempt.subjectId) {
          acc[dateKey].subjects.add(
            (attempt.subjectId as any)?.subjectName || 'Unknown',
          );
        }
      ...
      return {
        ...
        averageDailyQuizzes:
          trendData.reduce((sum, day) => sum + day.quizCount, 0) /
  ...
  private calculateOverallTrend(trendData: any[]): string {
    ...
    const recentAvg =
      recent.reduce((sum, day) => sum + day.averageScore, 0) / recent.length;
    const previousAvg =
      previous.reduce((sum, day) => sum + day.averageScore, 0) /
      previous.length;
  ...
  async updateUserGoalProgressAndWeakAreas(userId: string): Promise<void> {
    try {
      ...
      const targetScore =
        user.onboarding?.learningPreferences?.targetScore || 75;
      ...
      if (weakAreaCount > 0 && strongAreaCount > 0) {
        ...
        const weakAreaWeight = Math.min(
          0.8,
          0.4 + (weakAreaCount / performances.length) * 0.4,
        );
        const strongAreaWeight = 1 - weakAreaWeight;

        adjustedProgressScore =
          weakAreaAvg * weakAreaWeight + strongAreaAvg * strongAreaWeight;
      }
      ...
      const adjustedImprovementRate = Math.max(
        0,
        Math.min(100, baseImprovementRate + improvementBonus - declinesPenalty),
      );
      ...
      const goalProgress = {
        ...
        progressPercentage: Math.min(
          100,
          Math.round((adjustedProgressScore / targetScore) * 100),
        ),
        topicsAtTarget,
        ...
        weeklyGoalProgress: await this.calculateWeeklyGoalProgress(
          userId,
          weeklyGoal,
        ),
      };
      ...
      const focusAreaProgress = focusAreas
        .map((area) => {
          const areaPerformances = enhancedPerformances.filter(
            (p) =>
          ...
          const areaWeightedScore =
            areaPerformances.reduce((sum, p) => sum + p.weightedScore, 0) /
            areaPerformances.length;
          const areaImprovementTrend =
            areaPerformances.reduce((sum, p) => sum + p.improvementTrend, 0) /
            areaPerformances.length;
          ...
          return {
            ...
            trend:
              areaImprovementTrend > TREND_THRESHOLD
                ? 'improving'
                : areaImprovementTrend < -TREND_THRESHOLD
                  ? 'declining'
                  : 'stable',
            topicsInArea: areaPerformances.length,
            weakTopicsInArea: areaPerformances.filter((p) =>
              newWeakAreas.includes(p.topicName),
            ).length,
          };
        })
        .filter(Boolean);
      ...
      const prioritizedWeakAreas = newWeakAreas
        .map((topicName) => {
          const perf = enhancedPerformances.find(
      ...
      const updates = {
        ...
        'onboarding.progressMetrics.averageAccuracy':
          Math.round(currentAverageScore),
        'onboarding.progressMetrics.adjustedAccuracy': Math.round(
          adjustedProgressScore,
        ),
        'onboarding.progressMetrics.improvementRate': Math.round(
          adjustedImprovementRate,
        ),
        'onboarding.progressMetrics.questionsAnswered': analysis.totalAttempts,
        'onboarding.progressMetrics.engagementLevel':
          this.calculateEngagementLevel(analysis.totalAttempts, weeklyGoal),
      ...
      await this.userModel.updateOne({ _id: userObjectId }, { $set: updates });
      ...
      if (recentlyImprovedAreas.length > 0 || newlyWeakAreas.length > 0) {
        console.log(
          `Significant goal progress changes for user ${userId}:`,
          significantChanges,
        );
      }
   ...
   */
  private async calculateWeeklyGoalProgress(
    userId: string,
    weeklyGoal: number,
  ): Promise<any> {
    ...
    return {
      ...
      percentage: Math.min(
        100,
        Math.round((weeklyAttempts / weeklyGoal) * 100),
      ),
  ...
  private calculateImprovementRate(
    ...
    const totalTopics = performances.length;
    const topicsAtTarget = performances.filter(
      (p) => p.averageScore >= targetScore,
    ).length;
    return totalTopics > 0
      ? Math.round((topicsAtTarget / totalTopics) * 100)
      : 0;
  }
  ...
  async getUserGoalProgress(userId: string): Promise<any> {
    try {
      ...
      const aggregationResult = await this.userModel.aggregate([
        { $match: { _id: userObjectId } },
        {
          $project: {
            ...
            targetScore: {
              $ifNull: [
                '$onboarding.goalProgress.targetScore',
                '$onboarding.learningPreferences.targetScore',
                75,
              ],
            },
            currentAverageScore: {
              $ifNull: ['$onboarding.goalProgress.currentAverageScore', 0],
            },
            adjustedProgressScore: {
              $ifNull: [
                '$onboarding.goalProgress.adjustedProgressScore',
                '$onboarding.goalProgress.currentAverageScore',
                0,
              ],
            },
            progressPercentage: {
              $ifNull: ['$onboarding.goalProgress.progressPercentage', 0],
            },
            scoreGap: { $ifNull: ['$onboarding.goalProgress.scoreGap', 0] },
            topicsAtTarget: {
              $ifNull: ['$onboarding.goalProgress.topicsAtTarget', 0],
            },
            totalTopics: {
              $ifNull: ['$onboarding.goalProgress.totalTopics', 0],
            },
            weakAreasCount: {
              $ifNull: ['$onboarding.goalProgress.weakAreasCount', 0],
            },
            strongAreasCount: {
              $ifNull: ['$onboarding.goalProgress.strongAreasCount', 0],
            },
            recentlyImprovedCount: {
              $ifNull: ['$onboarding.goalProgress.recentlyImprovedCount', 0],
            },
            newlyWeakCount: {
              $ifNull: ['$onboarding.goalProgress.newlyWeakCount', 0],
            },
            weeklyGoalProgress: {
              $ifNull: [
                '$onboarding.goalProgress.weeklyGoalProgress',
                { target: 5, completed: 0, isOnTrack: false },
              ],
            },
            improvementRate: {
              $ifNull: [
                '$onboarding.goalProgress.improvementRate',
                '$onboarding.progressMetrics.improvementRate',
                0,
              ],
            },
            ...
            // Arrays
            focusAreaProgress: {
              $ifNull: ['$onboarding.focusAreaProgress', []],
            },
            weakAreas: {
              $ifNull: ['$onboarding.assessmentData.weakAreas', []],
            },
            strongAreas: {
              $ifNull: ['$onboarding.assessmentData.strongAreas', []],
            },
            improvingTopics: { $ifNull: ['$onboarding.improvingTopics', []] },
            decliningTopics: { $ifNull: ['$onboarding.decliningTopics', []] },
            recentlyImprovedAreas: {
              $ifNull: ['$onboarding.assessmentData.recentlyImprovedAreas', []],
            },
            newlyWeakAreas: {
              $ifNull: ['$onboarding.assessmentData.newlyWeakAreas', []],
            },

            // Additional fields
            focusAreas: {
              $ifNull: ['$onboarding.learningPreferences.focusAreas', []],
            },
            weeklyGoal: {
              $ifNull: ['$onboarding.learningPreferences.weeklyGoal', 5],
            },
          },
        },
      ]);
      ...
      const finalResult = {
        ...
        adjustedProgressScore:
          goalProgressData.adjustedProgressScore ||
          goalProgressData.currentAverageScore ||
          0,
        ...
        weakAreasCount:
          goalProgressData.weakAreasCount ||
          goalProgressData.weakAreas?.length ||
          0,
        strongAreasCount:
          goalProgressData.strongAreasCount ||
          goalProgressData.strongAreas?.length ||
          0,
        recentlyImprovedCount: goalProgressData.recentlyImprovedCount || 0,
        newlyWeakCount: goalProgressData.newlyWeakCount || 0,
        weeklyGoalProgress: goalProgressData.weeklyGoalProgress || {
          target: 5,
          completed: 0,
          isOnTrack: false,
        },
      ...
      console.log(
        `[getUserGoalProgress] Returning result for user ${userId}:`,
        finalResult,
      );
      return finalResult;
    } catch (error) {
